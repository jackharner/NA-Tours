...
const createSendToken = (user, statusCode, res) => {
    ...
    const cookieOptions = {
        ...
        httpOnly: true // not accessed by browser
    }
    if (process.env.Node_ENV === 'production') cookieOptions.secure = true; // only sent over secure connection like https
...
exports.protect = catchAsync(async (req, res, next) => {
    ...
    req.user = currentUser; // allow us to use the current user data in a later middleware function
    next();
});
...
exports.isLoggedIn = async (req, res, next) => {
    if (req.cookies.jwt) {
        try {
            ...
            ); // profisifying the verify method from the jwt module, then calling the verify method
            ...
            res.locals.user = currentUser // make data accessible to our templates
            return next();
        } catch (err) {
            return next();
        }
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
    ...
    await user.save({ validateBeforeSave: false }); // turn off validations such as passwordConfirm being set
    ...
    } catch (err) { // password reset process needs to be cancelled because email failed to send
